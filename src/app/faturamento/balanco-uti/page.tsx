...
export default async function BalancoDiarioUtiPage() {
  ...
  const passosAjuda = [
    { elemento: 'h1', titulo: 'Balanço Diário da UTI', texto: 'Semelhante ao balanço da Styllo, mas reservado apenas para as movimentações financeiras da UTI dos Óculos.' },
    { elemento: '.resumo-inferior', titulo: 'Fechamento de Caixa', texto: 'Acompanhe as Entradas menos as Compras e Saídas para descobrir o lucro líquido.' }
  ];
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto px-1 sm:px-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900 bg-gradient-to-r from-rose-600 to-red-700 bg-clip-text text-transparent">Balanço Diário (UTI)</h1>
          <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1">Conferência diária de compras, entradas e saídas da UTI.</p>
        </div>
        <Link href="/faturamento/balanco-uti/novo" className="w-full sm:w-auto text-center bg-rose-600 hover:bg-rose-700 active:scale-95 text-white px-5 py-3 rounded-xl font-black text-sm shadow-md transition-all duration-300 flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-rose-500 outline-none">
          <Plus className="h-4 w-4" /> Lançar na UTI
        </Link>
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-200/60 pb-2 overflow-x-auto scrollbar-none snap-x mask-linear-edge">
         ...
         <Link href="/faturamento/balanco-uti" className="px-4 py-2.5 bg-white border-t border-x border-slate-300 text-rose-600 font-black rounded-t-xl flex items-center gap-2 whitespace-nowrap shadow-sm border-b-2 border-b-white z-10 -mb-[2px]"><HeartPulse className="h-4 w-4" /> Balanço UTI</Link>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {registros.length === 0 ? (
          <div className="p-16 flex flex-col items-center justify-center text-center"><HeartPulse className="h-12 w-16 text-slate-300 mb-4 animate-pulse" /><h3 className="text-base font-black text-slate-900">Nenhum balanço da UTI registrado</h3></div>
        ) : (
          <div className="overflow-x-auto scrollbar-thin select-none">
            <table className="w-full text-center border-collapse min-w-[1000px]">
              <thead>
                <tr className="text-[10px] uppercase font-black text-rose-900 bg-rose-50/60 border-b border-slate-200">
              ...
              <tbody className="divide-y divide-slate-100 text-xs font-bold text-slate-800">
                {registros.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="p-3 border-r border-slate-100 text-rose-700">{item.entradaDinheiro > 0 ? `R$ ${item.entradaDinheiro.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-100 text-rose-700">{item.entradaCredito > 0 ? `R$ ${item.entradaCredito.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-100 text-rose-700">{item.entradaDebito > 0 ? `R$ ${item.entradaDebito.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-100 text-rose-700">{item.entradaPix > 0 ? `R$ ${item.entradaPix.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-100 text-red-600">{item.saidaPagamentos > 0 ? `R$ ${item.saidaPagamentos.toFixed(2)}` : '-'}</td>
                    <td className="p-2 bg-white"><BotoesAcao id={item.id} tabela="balanco-uti" caminho="/faturamento/balanco-uti" linkEditar={`/faturamento/balanco-uti/${item.id}/editar`} /></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                 <tr className="font-black text-[11px] text-slate-700 bg-slate-50 border-t border-slate-200">
                    ...
                    <td className="p-4 border-r border-slate-200 text-rose-700">R$ {sDinheiro.toFixed(2)}</td><td className="p-4 border-r border-slate-200 text-rose-700">R$ {sCredito.toFixed(2)}</td>
                    <td className="p-4 border-r border-slate-200 text-rose-700">R$ {sDebito.toFixed(2)}</td><td className="p-4 border-r border-slate-200 text-rose-700">R$ {sPix.toFixed(2)}</td>
                    <td className="p-4 border-r border-slate-200 text-red-700">R$ {sSaidas.toFixed(2)}</td><td className="p-4"></td>
                 </tr>
                 <tr className="font-black text-sm text-white bg-rose-600 text-center">
                    <td colSpan={2} className="p-4 border-r border-rose-500 uppercase tracking-widest text-right">TOTAL LÍQUIDO ACUMULADO:</td>
      ...
      {registros.length > 0 && (
        <div className="w-full sm:w-72 bg-white border border-slate-200 rounded-2xl shadow-sm p-5 space-y-3 text-sm font-black text-slate-800 resumo-inferior">
           <div className="flex justify-between items-center"><span className="text-slate-500 text-xs tracking-wider">ENTRADAS:</span><span className="text-rose-700 text-base">R$ {totalEntradas.toFixed(2)}</span></div>
           ...
           <div className="flex justify-between items-center pt-3 border-t border-slate-100 text-lg text-black mt-2"><span className="uppercase">TOTAL:</span><span className={totalLiquido >= 0 ? 'text-rose-700' : 'text-red-600'}>R$ {totalLiquido.toFixed(2)}</span></div>
        </div>
      )}
      <TutorialSistema passos={passosAjuda} idContexto="balanco-uti" />
    </div>
  );
}