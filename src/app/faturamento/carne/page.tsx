...
import { Plus, BookOpen, Wrench, Landmark, Activity, UserMinus, Handshake, Users, CalendarDays, Scale, Wallet } from 'lucide-react';
            ...
            <table className="w-full text-center border-collapse min-w-[2800px] border border-slate-400">
              <thead>
                ...
                <tr className="text-[10px] uppercase tracking-wider font-black text-black bg-slate-50">
                  ...
                  <th className="p-2 border border-slate-400">DATA DA COMPRA</th><th className="p-2 border border-slate-400">VALOR</th>
                  <th className="p-2 border border-slate-400">ENTRAD.</th>
                  {[...Array(10)].map((_, i) => (
                    <React.Fragment key={i}><th className="p-2 border border-slate-400 bg-yellow-50 text-yellow-900">{i+1}ª (R$)</th><th className="p-2 border border-slate-400 bg-yellow-50/50">DATA</th></React.Fragment>
                  ))}
              ...
              <tbody>
                {registros.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors font-bold text-[11px] text-slate-800">
                    ...
                    <td className="p-2 border border-slate-400">{item.dataCompra.split('-').reverse().join('/')}</td><td className="p-2 border border-slate-400 text-blue-700">R$ {item.valorVenda.toFixed(2)}</td>
                    ...
                    {[...Array(10)].map((_, i) => {
                      ...
                      return (<React.Fragment key={i}><td className="p-2 border border-slate-400 text-slate-600">{v > 0 ? v.toFixed(2) : '-'}</td><td className="p-2 border border-slate-400 text-slate-400 font-normal">{d ? d.split('-').reverse().join('/') : '-'}</td></React.Fragment>);
                    })}
              ...
              <tfoot>
                 ...
                 <tr className="bg-[#92d050] text-black font-black text-sm uppercase tracking-widest text-center">
                    ...
                    <td colSpan={19} className="p-3 border border-slate-400 text-left pl-8 text-lg">R$ {totalGeralTodasParcelas.toFixed(2)}</td><td className="p-3 border border-slate-400 bg-slate-200"></td>
                 </tr>
              </tfoot>
            </table>
          </div>
