  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/faturamento/conta-uti" className="p-2 hover:bg-slate-200 rounded-full text-slate-500"><ArrowLeft className="h-6 w-6" /></Link>
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Novo Valor Diário</h1>
          <p className="text-sm text-slate-500 font-medium">Lançamento na Conta UTI.</p>
        </div>
      </div>

      <form action={salvarContaUti} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        ...
        <div className="bg-[#e6f9fd] p-4 rounded-xl border border-[#00bdf2] flex justify-between items-center mt-6">
           <span className="font-black text-blue-900 uppercase tracking-widest flex items-center gap-2"><Calculator className="h-5 w-5" /> Total Final (UTI)</span>
           <span className="text-3xl font-black text-blue-900">R$ {total.toFixed(2).replace('.', ',')}</span>
        </div>

        <div className="flex justify-end pt-4"><button type="submit" className="bg-[#00bdf2] hover:bg-[#009bc2] px-8 py-3 rounded-xl font-black shadow-md"><Save className="h-5 w-5 inline mr-2"/> Salvar no Caixa UTI</button></div>
      </form>
    </div>
  );
}