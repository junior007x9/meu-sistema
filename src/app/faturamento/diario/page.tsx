...
import { Plus, CalendarDays, Wrench, Landmark, Activity, BookOpen, UserMinus, Handshake, Users } from 'lucide-react';
...
export default async function FaturamentoDiarioPage() {
  ...
  // TOTAIS MATEMÁTICOS (Igual ao Excel)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Faturamento Diário</h1>
          <p className="text-slate-500 mt-1">Gestão de entradas, saídas, dízimos e lucros.</p>
        </div>
        <Link href="/faturamento/diario/novo" className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-bold shadow-sm transition-transform hover:-translate-y-0.5">
          <Plus className="h-5 w-5" /> Novo Faturamento
        </Link>
      </div>

      {/* SISTEMA DE ABAS - 8 ABAS INTEGRADAS */}
      ...
      {/* BLOCO DE RESUMO FINANCEIRO (Igual ao Excel) */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-emerald-100 border border-emerald-300 p-4 rounded-xl shadow-sm text-center">
           <p className="text-xs font-black uppercase text-emerald-800">TOTAL BRUTO</p>
           <h3 className="text-2xl font-black text-emerald-900">R$ {sTotal.toFixed(2)}</h3>
        </div>
        <div className="bg-green-100 border border-green-300 p-4 rounded-xl shadow-sm text-center">
           <p className="text-xs font-black uppercase text-green-800">DÍZIMO (10%)</p>
           <h3 className="text-2xl font-black text-green-900">R$ {sDizimo.toFixed(2)}</h3>
        </div>
        <div className="bg-red-100 border border-red-300 p-4 rounded-xl shadow-sm text-center">
           <p className="text-xs font-black uppercase text-red-800">DESPESAS DE CAIXA</p>
           <h3 className="text-2xl font-black text-red-900">R$ {despesasCaixa.toFixed(2)}</h3>
        </div>
        <div className="bg-slate-200 border border-slate-400 p-4 rounded-xl shadow-sm text-center">
           <p className="text-xs font-black uppercase text-slate-800">TOTAL LÍQUIDO</p>
           <h3 className="text-2xl font-black text-slate-900">R$ {totalLiquido.toFixed(2)}</h3>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-300 shadow-sm overflow-hidden">
        {registros.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">Nenhum faturamento registrado</h3>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-center border-collapse min-w-[1400px] border border-slate-400">
              <thead>
                <tr>
                  <th colSpan={13} className="bg-slate-900 text-white py-3 font-black tracking-widest uppercase border border-slate-400 text-sm">
                    FATURAMENTO DIÁRIO
                  </th>
                </tr>
                ...
                <tr className="text-[9px] uppercase font-black text-black bg-slate-50">
                  <th className="p-2 border border-slate-400 w-24">R$ ESPÉCIE</th>
                  <th className="p-2 border border-slate-400 w-24">R$ CARTÃO<br/>CRÉDITO</th>
                  <th className="p-2 border border-slate-400 w-24">R$ CARTÃO<br/>DÉBITO</th>
                  <th className="p-2 border border-slate-400 w-24">R$ PIX</th>
                  <th className="p-2 border border-slate-400 w-20 text-red-600">R$</th>
                  <th className="p-2 border border-slate-400 w-20 text-red-600">PIX</th>
                </tr>
              </thead>
              <tbody>
                {registros.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors font-bold text-xs text-slate-800">
                    <td className="p-2 border border-slate-400">
                      {item.data.split('-').reverse().join('/')} <br/><span className="text-[8px] text-slate-400">{item.mesReferencia}</span>
                    </td>
                    ...
                    <td className="p-2 border border-slate-400">{item.especie.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{item.credito.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{item.debito.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{item.pix.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 bg-[#eaf1dd] text-green-900 font-black text-sm">{item.total.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 text-red-600">{item.saidaDinheiro.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 text-red-600">{item.saidaPix.toFixed(2)}</td>
                    ...
                    <td className="p-2 border border-slate-400 bg-white">
                      <BotoesAcao id={item.id} tabela="diario" caminho="/faturamento/diario" linkEditar={`/faturamento/diario/${item.id}/editar`} />
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                 <tr className="bg-slate-200 text-slate-900 font-black text-xs uppercase">
                    ...
                    <td className="p-2 border border-slate-400">{sEspecie.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{sCredito.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{sDebito.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400">{sPix.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 bg-[#c2d69b] text-green-900">{sTotal.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 text-red-700">{sSaidaDinheiro.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 text-red-700">{sSaidaPix.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 bg-green-200 text-green-900">{sDizimo.toFixed(2)}</td>
                    <td className="p-2 border border-slate-400 bg-blue-200 text-blue-900">{sFatEspecie.toFixed(2)}</td>
