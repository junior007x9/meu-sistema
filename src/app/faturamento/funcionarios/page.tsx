  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Link href="/faturamento/funcionarios/novo" className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-sm transition-transform hover:-translate-y-0.5"><Plus className="h-5 w-5" /> Lançar Despesas (Mês)</Link>
      </div>
      ...
      <div className="space-y-8">
        {registros.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-300 p-12 flex flex-col items-center justify-center text-center shadow-sm"><Users className="h-16 w-16 text-slate-300 mb-4" /><h3 className="text-lg font-bold text-slate-900">Nenhum mês de funcionário registrado</h3></div>
        ) : (
          registros.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white rounded-xl border border-slate-400 shadow-sm overflow-hidden">
                ...
                <div className="bg-[#d7e3bc] text-black text-center border-b border-slate-400 p-2 font-bold uppercase text-[11px] sm:text-sm relative">DESPESAS MÊS DE {item.mesReferencia} - ({item.nome})<div className="absolute right-2 top-1.5"><BotoesAcao id={item.id} tabela="funcionarios" caminho="/faturamento/funcionarios" linkEditar={`/faturamento/funcionarios/${item.id}/editar`} /></div></div>
                <div className="overflow-x-auto">
                  <table className="w-full text-center border-collapse min-w-[700px]">
                    <thead>
                      <tr className="bg-[#d7e3bc] text-black font-black text-xs"><th className="p-2 border border-slate-400 w-48 text-left" rowSpan={2}>DESPESAS</th><th className="p-2 border border-slate-400" colSpan={dias.length}>VALOR DIÁRIO</th><th className="p-2 border border-slate-400 bg-[#eaf1dd] w-24" rowSpan={2}>TOTAL</th></tr>
