...
import { Plus, Users, Wrench, Landmark, Activity, BookOpen, UserMinus, Handshake } from 'lucide-react';
...
export default async function FuncionariosPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* SISTEMA DE ABAS - FUNCIONÁRIOS ATIVO */}
      ...
      <div className="space-y-8">
        {registros.length === 0 ? (
          ...
          registros.map((item) => {
            ...
            // Regra do Excel: Se for 'F', mostra 'F'. Se for 0, deixa em branco.
            ...
            return (
              <div key={item.id} className="bg-white rounded-xl border border-slate-400 shadow-sm overflow-hidden">
                <div className="bg-[#c2d69b] text-black text-center border-b border-slate-400 p-2 font-black uppercase text-[11px] sm:text-sm">
                  ACOMPANHAMENTO DO ESTÁGIO DA FUNCIONÁRIO - INÍCIO: {item.dataInicio}
                </div>
                <div className="bg-[#d7e3bc] text-black text-center border-b border-slate-400 p-2 font-bold uppercase text-[11px] sm:text-sm relative">
                  DESPESAS MÊS DE {item.mesReferencia} - ({item.nome})
                  <div className="absolute right-2 top-1.5">
                     <BotoesAcao id={item.id} tabela="funcionarios" caminho="/faturamento/funcionarios" linkEditar={`/faturamento/funcionarios/${item.id}/editar`} />
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-center border-collapse min-w-[700px]">
                    <thead>
                      ...
                      <tr className="bg-[#d7e3bc] text-black font-black text-[11px]">
                        {dias.map((d: any, i: number) => (
                          <th key={i} className="p-1.5 border border-slate-400 w-10">{d.dia}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="text-[11px] sm:text-xs font-bold text-slate-800">
                      <tr>
                        <td className="p-1.5 border border-slate-400 text-left">VALE TRANSPORTE</td>
                        {dias.map((d: any, i: number) => <td key={i} className="p-1.5 border border-slate-400">{renderCell(d.status, d.vt)}</td>)}
                        <td className="p-1.5 border border-slate-400">{item.totalVt.toFixed(2).replace('.', ',')}</td>
                      </tr>
                      <tr>
                        <td className="p-1.5 border border-slate-400 text-left">VALE ALIMENTAÇÃO</td>
                        {dias.map((d: any, i: number) => <td key={i} className="p-1.5 border border-slate-400">{renderCell(d.status, d.va)}</td>)}
                        <td className="p-1.5 border border-slate-400">{item.totalVa.toFixed(2).replace('.', ',')}</td>
                      </tr>
                      <tr>
                        <td className="p-1.5 border border-slate-400 text-left">SALÁRIO COMERCIAL</td>
                        {dias.map((d: any, i: number) => <td key={i} className="p-1.5 border border-slate-400">{renderCell(d.status, d.sal)}</td>)}
                        <td className="p-1.5 border border-slate-400">{item.totalSalario.toFixed(2).replace('.', ',')}</td>
                      </tr>
                      <tr>
                        <td className="p-1.5 border border-slate-400 text-left">FÉRIAS</td>
                        {dias.map((d: any, i: number) => <td key={i} className="p-1.5 border border-slate-400">{renderCell(d.status, d.fer)}</td>)}
                        <td className="p-1.5 border border-slate-400">{item.totalFerias.toFixed(2).replace('.', ',')}</td>
                      </tr>
                      <tr>
                        <td className="p-1.5 border border-slate-400 text-left">13º SALÁRIO</td>
                        {dias.map((d: any, i: number) => <td key={i} className="p-1.5 border border-slate-400">{renderCell(d.status, d.d13)}</td>)}
                        <td className="p-1.5 border border-slate-400">{item.total13.toFixed(2).replace('.', ',')}</td>
                      </tr>
                    </tbody>
                    <tfoot>
                      <tr className="text-sm font-black text-black">
                        <td colSpan={dias.length + 1} className="p-2 border border-slate-400 text-right uppercase bg-white">TOTAL</td>
                        <td className="p-2 border border-slate-400 bg-white">{item.totalGeral.toFixed(2).replace('.', ',')}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
