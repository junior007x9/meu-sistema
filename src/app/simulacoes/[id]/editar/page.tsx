...
export default async function EditarSimulacaoPage({ params }: any) {
  const resolvedParams = await params;
  const simId = parseInt(resolvedParams.id);
  
  const resultado = await db.select().from(simulacoesLentes).where(eq(simulacoesLentes.id, simId));
  const sim = resultado[0];
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div><h1 className="text-3xl font-bold text-white">Editar Simulação</h1></div>
      </div>
      <form action={atualizarSimulacao} className="bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-700 space-y-4 text-white">
        ...
        <p className="text-xs text-slate-500 mt-4">* Os resultados serão recalculados automaticamente pelo sistema.</p>
        <div className="flex justify-end pt-4"><button type="submit" className="bg-yellow-500 text-slate-900 hover:bg-yellow-600 px-8 py-3 rounded-xl font-bold shadow-md"><Save className="h-5 w-5 inline mr-2"/> Atualizar Lançamento</button></div>
      </form>
    </div>
  );
}