...
export default async function EditarOSPage({ params }: any) {
  const resolvedParams = await params;
  const osId = parseInt(resolvedParams.id);
  
  const resultado = await db.select().from(ordensServico).where(eq(ordensServico.id, osId));
  const osData = resultado[0];
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div><h1 className="text-3xl font-bold text-slate-900">Editar Ordem de Serviço</h1></div>
      </div>
      <form action={atualizarOS} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select name="clienteId" defaultValue={osData.clienteId || ''} required className="w-full px-4 py-2 bg-slate-50 border rounded-lg outline-none">
          ...
          <div>
            ...
            <select name="tecnico" defaultValue={osData.tecnico || ''} className="w-full px-4 py-2 bg-emerald-50 border border-emerald-300 font-bold rounded-lg outline-none">
        ...
        <div>
          ...
          <input type="text" name="servicoRealizado" defaultValue={osData.servicoRealizado || ''} className="w-full px-4 py-2 bg-slate-50 border rounded-lg" />
        </div>
        ...
        <div className="flex justify-end pt-4"><button type="submit" className="bg-yellow-500 hover:bg-yellow-600 text-slate-900 px-8 py-3 rounded-xl font-bold shadow-md flex items-center gap-2"><Save className="h-5 w-5"/> Salvar Alterações</button></div>
      </form>
    </div>
  );
}