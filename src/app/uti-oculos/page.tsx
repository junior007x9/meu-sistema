...
import { Plus, Wrench, Wallet, QrCode, CreditCard } from 'lucide-react';
import BotoesAcao from '@/components/BotoesAcao'; // <-- Importamos o nosso novo botão!
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {listaOS.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-center">
            <div className="h-16 w-16 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mb-4"><Wrench className="h-8 w-8" /></div>
            ...
            <table className="w-full text-left border-collapse min-w-[1200px]">
              <thead>
                <tr className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-600 font-black border-b border-slate-200">
                  ...
                  <th className="p-3 text-center bg-yellow-50 text-yellow-900 border-r border-slate-200" rowSpan={2}>TOTAL</th>
                  <th className="p-3 text-center bg-slate-100 text-slate-900 border-b border-slate-200" rowSpan={2}>AÇÕES</th>
                </tr>
              ...
              <tbody className="divide-y divide-slate-200">
                {listaOS.map((os) => (
                  <tr key={os.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3 border-r border-slate-200 text-xs font-bold text-slate-500 text-center">{os.data ? new Date(os.data).toLocaleDateString('pt-BR') : '-'}</td>
                    <td className="p-3 border-r border-slate-200 text-sm font-bold text-slate-900">{os.clienteNome || '-'}</td>
                    <td className="p-3 border-r border-slate-200 text-xs font-black text-slate-800 text-center bg-slate-50">{os.id.toString().padStart(5, '0')}</td>
                    <td className="p-3 border-r border-slate-200 text-xs font-medium text-slate-700">{os.servicoRealizado || os.defeito}</td>
                    <td className="p-3 border-r border-slate-200 text-xs font-bold text-slate-600 text-right">{os.valorPix > 0 ? `R$ ${os.valorPix.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-200 text-xs font-bold text-slate-600 text-right">{os.valorEspecie > 0 ? `R$ ${os.valorEspecie.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-200 text-xs font-bold text-slate-600 text-right">{os.valorCartao > 0 ? `R$ ${os.valorCartao.toFixed(2)}` : '-'}</td>
                    <td className="p-3 border-r border-slate-200 text-sm font-black text-slate-900 text-right bg-yellow-50/30">R$ {os.valorTotal.toFixed(2)}</td>
                    
                    {/* AQUI ESTÁ O NOSSO NOVO BOTÃO RENDERIZADO */}
                    <td className="p-3 bg-slate-50 border-slate-200">
                      <BotoesAcao 
                        id={os.id} 
                        tabela="os" 
                        caminho="/uti-oculos" 
                        linkEditar={`/uti-oculos/${os.id}/editar`} 
                      />
                    </td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr className="bg-slate-900 text-white font-black text-sm uppercase tracking-wider border-t-4 border-yellow-500">
                  <td colSpan={4} className="p-4 text-right text-yellow-500">SOMA TOTAL DO MÊS:</td>
                  <td className="p-4 text-right border-x border-slate-700">R$ {somaPix.toFixed(2)}</td>
                  <td className="p-4 text-right border-r border-slate-700">R$ {somaEspecie.toFixed(2)}</td>
                  <td className="p-4 text-right border-r border-slate-700">R$ {somaCartao.toFixed(2)}</td>
                  <td className="p-4 text-right text-yellow-500 bg-black/20">R$ {somaGeral.toFixed(2)}</td>
                  <td className="bg-black/20 border-l border-slate-700"></td>
                </tr>
              </tfoot>
            </table>
          </div>
